...
# Vertex + qBittorrent 4.3.9 组合安装脚本
...
install_qb439() {
    ...
    # 下载并执行qb439安装脚本
    wget -O /tmp/qb439_temp.sh ${GITHUB_RAW_URL}/scripts/install/qb439.sh
    chmod +x /tmp/qb439_temp.sh
    
    # 执行安装脚本
    bash /tmp/qb439_temp.sh
    
    # 清理临时文件
    rm -f /tmp/qb439_temp.sh
...
show_info() {
    ...
    echo -e "4. qBittorrent 4.3.9包含性能优化和BBR加速"
    echo
    echo -e "${GREEN}================================================${NC}"
}
...
# 主函数
main() {
    echo -e "${CYAN}开始安装Vertex + qBittorrent 4.3.9...${NC}"
